...
function solveLinearSystem(matrix: number[][], load: number[]): number[] {
    ...
    for (let pivot = 0; pivot < size; pivot++) {
        ...
        if (Math.abs(pivotValue) < 1e-12) {
            throw new Error('Singular stiffness matrix during portal frame analysis.');
        }
...
function transformationMatrix(directionCosineX: number, directionCosineZ: number): number[][] {
...
function elementGeometry(nodeI: Node2D, nodeJ: Node2D): {
    length: number;
...
function assembleGlobalStiffness(
    ...
    const stiffness = Array.from({ length: dofCount }, () => Array<number>(dofCount).fill(0));
    ...
    for (const element of elements) {
        ...
        for (let row = 0; row < 6; row++) {
            for (let col = 0; col < 6; col++) {
                stiffness[dofMap[row]][dofMap[col]] += globalElementStiffness[row][col];
            }
        }
    }
...
function expandDisplacements(freeDofs: number[], solution: number[], dofCount: number): number[] {
...
function buildFrameModel(members: FrameMember[]): {
    ...
    const nodes: Node2D[] = [];
    const nodeKey = (x: number, z: number): string => `${x.toFixed(4)}:${z.toFixed(4)}`;
...
export function analyzePortalFrame(built: BuiltPortalFrame): FrameAnalysisResult {
    ...
    const displacements = expandDisplacements(freeDofs, freeDisplacements, nodes.length * 3);
    ...
    const apexNodeIndex = nodes.find((node) => Math.abs(node.x) < 1e-4 && node.z > 1)?.id;
