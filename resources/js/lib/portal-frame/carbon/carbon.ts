...
import type { CarbonFactors } from '@/lib/portal-frame/carbon/carbon-factors';
 ...
 * haunch length, so its mass is estimated as half of an equivalent run over 10%
 * of the frame span.
 */
...
function foundationConcreteVolumeM3(member: FrameMember): number {
    if (member.footing) {
        return (
            member.footing.width * member.footing.depth * member.footing.height
        );
    }

    const pileVolume = member.pile
        ? (Math.PI / 4) * member.pile.diameter ** 2 * member.pile.depth
        : 0;

    return pileVolume;
}

function slabConcreteMassKg(
...
function foundationSizingForDesign(
    ...
    try {
        ...
        return sizeFoundationReactions(reactions, design, built.column);
    } catch {
        return null;
    }
}
        ...
        switch (member.role) {
            ...
            case 'foundation': {
                ...
                if (foundationSizing && isMainColumnPad(member)) {
                    ...
                    const sizing = foundationSizing[side];
                    const rebarMassKg = sizing.rebarMassKg ?? 0;
