...
import { carbonFactors } from '@/lib/portal-frame/carbon/carbon-factors';
...
import { HAUNCH_LENGTH_FRACTION } from '@/lib/portal-frame/model/haunch-geometry';
...
describe('calculatePortalFrameCarbon', () => {
    ...
    it('applies the galvanized steel factor to side rails and purlins', () => {
        const { breakdown } = calculatePortalFrameCarbon(
    ...
    it('adds a 250mm ground floor slab concrete allowance', () => {
        ...
        const concreteDensityKgM3 =
            (design.foundation.assumptions.concreteDensityKnM3 * 1000) /
            9.80665;
    ...
    it('estimates haunch mass as a tapered fraction of 10% of span', () => {
        const design = defaultPortalFrameDesign();
        const { breakdown } = calculatePortalFrameCarbon(design);
        const built = buildPortalFrame(design);
        const rafter = built.members.find((member) => member.role === 'rafter');

        expect(rafter).toBeDefined();

        expect(breakdown.haunches.massKg).toBeCloseTo(
            breakdown.rafters.massKg *
                (design.span / memberLengthM(rafter!)) *
    ...
    it('counts pile-cap rebar from the sizer reinforcement rate', () => {
        ...
        // Two-pile caps now model a reinforcement rate (kg/m3) of concrete volume,
        // so the rebar line is non-zero for pile-cap foundations.
        expect(breakdown.rebar.massKg).toBeGreaterThan(0);
        expect(breakdown.rebar.carbonKg).toBeGreaterThan(0);
        expect(breakdown.concrete.massKg).toBeGreaterThan(0);
    });

    it('counts no rebar for plain mass-filled foundations', () => {
        const design: PortalFrameDesign = {
            ...defaultPortalFrameDesign(),
            foundation: {
                ...defaultPortalFrameDesign().foundation,
