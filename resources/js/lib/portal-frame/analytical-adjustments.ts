import { buildAnalyticalGableColumns } from '@/lib/portal-frame/gable-columns';
import { isUbSection } from '@/lib/portal-frame/member-basis';
import { buildAnalyticalPurlins } from '@/lib/portal-frame/purlins';
import { buildAnalyticalSideRails } from '@/lib/portal-frame/side-rails';
import type {
    CSectionDimensions,
    FrameMember,
    PortalFrameDesign,
...
function isZSection(section: FrameMember['section']): section is ZSectionDimensions {
    return section.profile === 'z';
}

function isCSection(section: FrameMember['section']): section is CSectionDimensions {
...
export function adjustMembersForAnalysis(
    ...
    const sideRailMember = members.find((member) => member.role === 'side_rail');
    const gableColumnMember = members.find((member) => member.role === 'gable_column');
