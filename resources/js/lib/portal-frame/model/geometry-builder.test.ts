...
describe('buildPortalFrame', () => {
    it('adds longitudinal UB 203 eaves ties along each side of the building', () => {
        ...
        const rightTie = ties.find(
            (member) => member.id === 'eaves-tie-right',
        )!;
    ...
    it('includes purlins and side rails with the configured secondary sections', () => {
        const built = buildPortalFrame(defaultPortalFrameDesign());
        const purlins = built.members.filter(
            (member) => member.role === 'purlin',
        );
        const sideRails = built.members.filter(
        ...
        expect(
            built.members.filter((member) =>
                member.id.startsWith('gable-front-column-'),
            ),
        ).toHaveLength(3);
        expect(
            built.members.filter((member) =>
    ...
    it('adds two 450 mm x 6 m piles under each two-pile cap', () => {
        ...
        expect(
            Math.abs(firstLeftPiles[1]!.start[0] - firstLeftPiles[0]!.start[0]),
        ).toBeCloseTo(0.45 * 3);
    });
});
