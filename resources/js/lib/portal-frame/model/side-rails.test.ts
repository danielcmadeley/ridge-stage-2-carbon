...
} from '@/lib/portal-frame/model/side-rails';
...
describe('buildSideRails', () => {
    ...
    it('places four side rails per wall up to the eaves height', () => {
        const sideRails = buildSideRails(
            design,
            section,
            built.column,
            gableColumnSection,
    ...
    it('positions rails at 1.0 m then every 1.5 m', () => {
        const leftHeights = buildSideRails(
            design,
            section,
            built.column,
            gableColumnSection,
    ...
    it('anchors each side rail outside the column outer flange', () => {
        const outerX = outerColumnFlangeX(
            design.span / 2,
            built.column,
            'left',
        );
        ...
        const sideRail = buildSideRails(
            design,
            section,
            built.column,
            gableColumnSection,
        ).find((member) => member.id === 'side-rail-left-0')!;
    ...
    it('anchors gable side rails outside the gable column outer flange', () => {
        ...
        const frontAnchorY = sideRailGableAnchorY(
            0,
            gableColumnSection,
            section,
            'front',
        );
        const rearAnchorY = sideRailGableAnchorY(
            40,
            gableColumnSection,
            section,
            'rear',
        ...
        expect(
            buildSideRails(
                ...
                built.column,
                gableColumnSection,
            ).find((member) => member.id === 'side-rail-gable-rear-0')!
                .start[1],
        ).toBe(rearAnchorY);
    });
    ...
    it('wraps side rails around the front and rear gable ends', () => {
        ...
        expect(
            gableRails.find((member) => member.id === 'side-rail-gable-rear-0')!
                .start[1],
        ).toBe(rearAnchorY);
        expect(
            gableRails.find(
