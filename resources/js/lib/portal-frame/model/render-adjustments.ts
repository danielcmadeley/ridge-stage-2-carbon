...
function trimMemberAtColumnFace(
    ...
    const innerFaceX = columnX + Math.sign(-columnX || 1) * halfFlangeWidthM;
...
export function extendColumnToRafterTop(
    ...
    return {
        ...column,
        end: [column.end[0], column.end[1], column.end[2] + verticalExtension],
    };
}
 ...
 */
export function adjustMembersForRendering(
    members: FrameMember[],
): FrameMember[] {
    ...
    for (const member of members) {
        ...
        trimmedRafters.set(member.id, trimRafterAtColumnFace(member, column));
    }
