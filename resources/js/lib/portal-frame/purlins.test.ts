...
describe('buildPurlins', () => {
    ...
    it('places eight purlins per roof slope for the default design', () => {
        ...
        expect(purlins.filter((member) => member.role === 'purlin')).toHaveLength(16);
        expect(purlins.filter((member) => member.id.startsWith('purlin-left-'))).toHaveLength(8);
        expect(purlins.filter((member) => member.id.startsWith('purlin-right-'))).toHaveLength(8);
    });
