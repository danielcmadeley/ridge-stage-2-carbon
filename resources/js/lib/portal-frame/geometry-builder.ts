...
import { buildGableEndBracing } from '@/lib/portal-frame/gable-bracing';
import { buildGableColumns } from '@/lib/portal-frame/gable-columns';
import { buildPurlins } from '@/lib/portal-frame/purlins';
import { buildSideRails } from '@/lib/portal-frame/side-rails';
...
import { findChsSection } from '@/lib/portal-frame/chs-sections';
import { findUbSection } from '@/lib/portal-frame/ub-sections';
import { findZSection } from '@/lib/portal-frame/z-sections';
import type {
...
function buildMembers(
    ...
    for (let frameIndex = 0; frameIndex < frameCount; frameIndex++) {
        ...
        for (const side of [-1, 1] as const) {
            ...
            const footing = side < 0 ? foundationDimensions.left : foundationDimensions.right;
            const foundationResult = side < 0 ? foundationSizing?.left : foundationSizing?.right;
                ...
                for (const pileIndex of [-1, 1] as const) {
                    ...
                    members.push({
                        ...
                        start: [pileX, y, -footing.heightM],
                            ...
                            pileX,
                            y,
                            -footing.heightM - foundationResult.pileCap.pileDepthM,
                        ],
                        section: footingSection,
                        pile: {
    ...
    members.push(...buildGableColumns(design, gableColumnSection, rafterSection));
    members.push(...buildSideRails(design, sideRailSection, columnSection, gableColumnSection));
