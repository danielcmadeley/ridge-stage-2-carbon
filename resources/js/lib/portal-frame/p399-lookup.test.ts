...
import { lookupP399Section } from '@/lib/portal-frame/p399-lookup';
...
describe('lookupP399Section', () => {
    ...
    it('flags line loads beyond the tabulated range as out of scope', () => {
        expect(() => lookupP399Section('Rafter', 18, 6, 20)).toThrow(/out of scope/i);
    });

    it('flags eaves heights beyond the tabulated range as out of scope', () => {
        expect(() => lookupP399Section('Rafter', 10, 13, 20)).toThrow(/out of scope/i);
    });

    it('flags spans beyond the tabulated range as out of scope', () => {
        expect(() => lookupP399Section('Rafter', 10, 6, 45)).toThrow(/out of scope/i);
        expect(() => lookupP399Section('Rafter', 10, 6, 12)).toThrow(/out of scope/i);
    });
