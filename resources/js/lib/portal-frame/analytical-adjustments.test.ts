...
import { pointOnRafter, pointOnRafterAtX, rafterUndersideZAtX } from '@/lib/portal-frame/purlins';
...
describe('adjustMembersForAnalysis', () => {
    ...
    it('places purlins on the rafter centreline instead of the physical anchor offset', () => {
        ...
        const renderPurlin = built.members.find((member) => member.id === 'purlin-left-0')!;
    ...
    it('places wall side rails on the column centreline', () => {
        const members = adjustMembersForAnalysis(built.members, design);
        const renderSideRail = built.members.find((member) => member.id === 'side-rail-left-0')!;
        ...
        expect(Math.abs(analysisSideRail.start[0])).toBeLessThan(Math.abs(renderSideRail.start[0]));
    });

    it('splits purlins and wall side rails at every frame position', () => {
        const members = adjustMembersForAnalysis(built.members, design);
        const purlins = members.filter((member) => member.role === 'purlin');
        const wallSideRails = members.filter((member) =>
            member.id.startsWith('side-rail-left-') || member.id.startsWith('side-rail-right-'),
        );

        expect(purlins).toHaveLength(16 * bayCount);
        expect(wallSideRails).toHaveLength(8 * bayCount);
        expect(
            members.find((member) => member.id === 'purlin-left-0-bay-0')!.end[1],
        ).toBe(design.baySpacing);
        expect(
            members.find((member) => member.id === 'side-rail-right-0-bay-0')!.start[1],
        ).toBe(0);
        expect(
            members.find((member) => member.id === 'side-rail-right-0-bay-0')!.end[1],
        ).toBe(design.baySpacing);
    });
    ...
    it('adds nodes at every frame intersection for secondary members', () => {
        ...
        const framePositions = Array.from({ length: bayCount + 1 }, (_, index) =>
            index * design.baySpacing,
        );
    ...
    it('splits gable side rails at every gable column position', () => {
        ...
        expect(
            members.find((member) => member.id === 'side-rail-gable-front-0-seg-0')!.start[0],
        ).toBe(gableColumnPositions[0]);
        expect(
            members.find((member) => member.id === 'side-rail-gable-front-0-seg-0')!.end[0],
        ).toBe(gableColumnPositions[1]);
        expect(
            members.find((member) => member.id === 'side-rail-gable-front-0-seg-0')!.start[1],
        ).toBe(0);
    });
    ...
    it('leaves the solid render member list unchanged', () => {
        expect(built.members.find((member) => member.id === 'purlin-left-0')!.end[1]).toBe(40);
        expect(built.members.find((member) => member.id === 'side-rail-left-0')!.start[0]).not.toBe(
            -halfSpan,
        );
        expect(
            built.members.find((member) => member.id === 'side-rail-gable-front-0')!.start[0],
        ).not.toBe(-halfSpan);
        expect(
            built.members.find((member) => member.id === 'gable-front-column-1')!.end[2],
