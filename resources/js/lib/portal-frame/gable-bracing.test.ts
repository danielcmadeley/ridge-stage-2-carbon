...
} from '@/lib/portal-frame/gable-bracing';
...
describe('buildGableEndBracing', () => {
    it('returns no members when there is only one frame', () => {
        expect(
            buildGableEndBracing(1, 5, 12, 6, 7.26, braceSection),
        ).toEqual([]);
    });
...
describe('buildPortalFrame gable bracing integration', () => {
    it('includes bracing members in the built frame', () => {
        ...
        const braces = built.members.filter((member) => member.role === 'brace');

        expect(braces).toHaveLength(16);
        expect(braces.every((member) => member.section.name === '114.3x5.0 CHS')).toBe(
            true,
        );
    });
});
