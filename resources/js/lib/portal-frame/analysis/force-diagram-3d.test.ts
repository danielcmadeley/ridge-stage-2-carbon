...
import { collectForceDiagramObjects } from '@/lib/portal-frame/analysis/force-diagram-hover';
...
function frameZeroMember(
    built: ReturnType<typeof buildPortalFrame>,
    id: string,
): FrameMember {
...
describe('moment diagram rendering', () => {
    it('plots opposite members as mirror images, not inversions', () => {
        ...
        const byId = new Map(
            analysis.members.map((member) => [member.id, member]),
        );
        ...
        for (const pair of [
            ...
            for (
                let station = 0;
                station < leftAnalysis.momentKnm.length;
                station++
            ) {
    ...
    it('attaches hover metadata to rendered force diagram objects', () => {
        ...
        for (const target of hoverTargets) {
            ...
            expect(
                target.userData.forceDiagramHover.analysis.stationsM.length,
            ).toBeGreaterThan(0);
        }
    });
    ...
    it('uses smaller moments on gable end frames than interior frames', () => {
        ...
        const gableHover = hoverTargets.find(
            (target) =>
                target.userData.forceDiagramHover.member.id ===
                'frame-0-column-left',
        );
        ...
        const interiorEavesMoment = Math.abs(
            interiorHover!.userData.forceDiagramHover.analysis.momentKnm.at(
                -1,
            )!,
        );
...
describe('shear and axial diagram rendering', () => {
    it.each(['shear', 'axial'] as const)(
        ...
        (mode: AnalyticalForceMode) => {
            ...
            const hoverTargets = collectForceDiagramObjects(group);
            const fillMeshes = hoverTargets.filter(
                (target) => target instanceof Mesh,
            );
