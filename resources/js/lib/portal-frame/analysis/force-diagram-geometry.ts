 ...
 */
export function pointAlongMember(
    member: MemberLine,
    distanceM: number,
): Point2D {
...
export function closedForceDiagramPolygon(
    ...
    const centreline = stationsM.map((station) =>
        pointAlongMember(member, station),
    );
...
export function forceDiagramScale(
    ...
    const spanExtent = members.reduce((max, member) => {
        ...
        return Math.max(max, Math.max(...xs.map(Math.abs)), Math.max(...zs));
    }, 1);
