...
export function isForceDiagramHoverObject(
    ...
    userData: { [FORCE_DIAGRAM_HOVER_KEY]: ForceDiagramHoverUserData };
} {
...
export function formatForceValue(
    mode: AnalyticalForceMode,
    value: number,
): string {
    if (mode === 'moment') {
...
export function stationFromLocalPoint(
    member: FrameMember,
    localPoint: Vector3,
): number {
...
export function resolveForceDiagramHover(
    ...
    const { member, analysis, mode } =
        intersection.object.userData[FORCE_DIAGRAM_HOVER_KEY];
    const stationM = stationFromLocalPoint(member, frameGroupLocalPoint);
    const values = valuesForForceMode(analysis, mode);
    const forceValue = interpolateForceAtStation(
        analysis.stationsM,
        values,
        stationM,
    );
