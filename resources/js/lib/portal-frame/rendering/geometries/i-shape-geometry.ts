...
function topAnchoredIProfilePoints(
    ...
    if (depthM <= 1e-9) {
        return Array.from(
            { length: BOTTOM_ANCHORED_I_PROFILE_VERTEX_COUNT },
            () => [0, 0],
        );
    }
...
export function createTaperedTopAnchoredIShapeGeometry(
    ...
    for (let ring = 0; ring < ringCount; ring += 1) {
        ...
        const depthM =
            baseDepthM *
            (startDepthFactor + (endDepthFactor - startDepthFactor) * t);
        const z = t * lengthM;
    ...
    for (let ring = 0; ring < segments; ring += 1) {
        ...
        const nextRingStart =
            (ring + 1) * BOTTOM_ANCHORED_I_PROFILE_VERTEX_COUNT;

        for (
            let vertex = 0;
            vertex < BOTTOM_ANCHORED_I_PROFILE_VERTEX_COUNT;
            vertex += 1
        ) {
            const nextVertex =
                (vertex + 1) % BOTTOM_ANCHORED_I_PROFILE_VERTEX_COUNT;
...
export function createFrameMemberMesh(member: FrameMember): Mesh {
    ...
    if (member.role === 'foundation' && member.pile) {
        const geometry = createPileGeometry(
            member.pile.diameter,
            member.pile.depth,
        );
    ...
    const material = createFrameMemberSteelMaterial(member);
    const mesh = new Mesh(geometry, material);
