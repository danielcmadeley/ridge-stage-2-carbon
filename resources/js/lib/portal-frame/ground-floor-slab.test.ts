...
import { buildPortalFrameThreeGroup } from '@/lib/portal-frame/three-group';
...
describe('groundFloorSlab', () => {
    ...
    it('adds the slab to the solid portal frame group', () => {
        const group = buildPortalFrameThreeGroup(defaultPortalFrameDesign(), 'solid');

        expect(group.children.some((child) => child.name === 'ground-floor-slab')).toBe(true);
    });
});
