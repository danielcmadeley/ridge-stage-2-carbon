...
import { queryTerrainAltitude } from '@/lib/uk-map-terrain';
...
export function setupBuildingDrag(
    ...
    const onMouseDown = (event: MapMouseEvent): void => {
        ...
        const buildingId = pickBuildingAtPoint(map, buildings.value, event.point);
    ...
    const onMouseMove = (event: MapMouseEvent): void => {
        if (!draggingId) {
            const hoveredId = pickBuildingAtPoint(map, buildings.value, event.point);
