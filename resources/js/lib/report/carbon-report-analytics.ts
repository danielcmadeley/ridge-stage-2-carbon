...
import { SCORS_BANDS } from '@/lib/portal-frame/carbon/scors';
...
function buildScorsBenchmark(
    ...
    return SCORS_BANDS.map((definition) => {
        ...
        if (definition.band === currentBand) {
            distanceKgM2 = upperBound === null ? 0 : upperBound - intensityKgM2;
        }
...
export function typstChartDataRows(
    ...
    return rows
        .map((row) => typstChartDataRow(row.label, row.carbonKg))
        .join('');
}
